...
const els = {
  ...
  registerButton: document.querySelector("#registerForm button[type='submit']"),
...
async function start() {
  window.addEventListener("error", (event) => {
    setAuthStatus(`JavaScript error: ${event.message}`, "error");
  });
  window.addEventListener("unhandledrejection", (event) => {
    setAuthStatus(`App error: ${event.reason?.message || event.reason}`, "error");
  });

  applyTheme();
  bindEvents();
  els.loginForm.noValidate = true;
  els.registerForm.noValidate = true;
  setAuthStatus("JavaScript loaded. Ready to login/register.", "success");
...
async function register(event) {
  event.preventDefault();
  setButtonLoading(els.registerButton, true, "Creating...");
  setAuthStatus("Creating account...", "success");
  try {
    const name = els.registerName.value.trim();
    const email = els.registerEmail.value.trim();
    const password = els.registerPassword.value;
    if (!name) throw new Error("Name required hai");
    if (!email || !email.includes("@")) throw new Error("Valid email daalo");
    if (password.length < 6) throw new Error("Password at least 6 characters ka rakho");
    ...
    setAuthStatus("Account created. Opening dashboard...", "success");
    ...
    setButtonLoading(els.registerButton, false, "Create Account");
  }
}
...
async function login(event) {
  ...
  setAuthStatus("Logging in...", "success");
  try {
    const email = els.loginIdentity.value.trim();
    const password = els.loginPassword.value;
    if (!email || !email.includes("@")) throw new Error("Valid email daalo");
    if (!password) throw new Error("Password daalo");
    ...
    setButtonLoading(els.loginButton, false, "Login");
  }
}
...
async function enterApp() {
  ...
  try {
    ...
    projects = [];
    activity = [];
    showToast("Login ho gaya, but Firestore data load nahi hua. Rules check karo.");
  }
  render();
}
...
async function firestoreRequest(path, options = {}) {
  ...
  const response = await fetch(`${firestoreBase}${separator}${path}`, {
    ...options,
    headers: {
...
function readableError(error) {
  ...
  if (message.includes("Missing or insufficient permissions")) {
    return "Firestore rules publish nahi hue ya permission issue hai";
  }
  if (message.includes("Failed to fetch")) {
    return "Firebase se connect nahi ho raha. Internet/domain check karo";
  }
...
function setAuthStatus(message, type = "") {
  ...
  if (type) els.authStatus.classList.add(type);
}
